...
func encode_utf8(buf []byte, c uint32) int {
	if c <= 0x7F {
		buf[0] = byte(c)
		return 1
	}
	...
	if c <= 0x7FF {
		buf[0] = byte(0b11000000 | (c >> 6))
		buf[1] = byte(0b10000000 | (c & 0b00111111))
		return 2
	}

	if c <= 0xFFFF {
		buf[0] = byte(0b11100000 | (c >> 12))
		buf[1] = byte(0b10000000 | ((c >> 6) & 0b00111111))
		buf[2] = byte(0b10000000 | (c & 0b00111111))
		return 3
	}

	buf[0] = byte(0b11110000 | (c >> 18))
	buf[1] = byte(0b10000000 | ((c >> 12) & 0b00111111))
	buf[2] = byte(0b10000000 | ((c >> 6) & 0b00111111))
	buf[3] = byte(0b10000000 | (c & 0b00111111))
	return 4
}
...
// than one byte.
func decode_utf8(buf []byte, pos int) (uint32, int, error) {
	chr := buf[pos]
	if chr < 128 {
		return uint32(chr), pos + 1, nil
	}
	...
	for i := 1; i < len_; i++ {
		...
		if cc>>6 != 0b10 {
			...
			return 0, 0, errors.New("invalid UTF-8 sub sequence")
		}

		c = (c << 6) | uint32(cc&0b111111)
	}

	return c, pos + len_, nil
}

func in_range(rangePairs []uint32, c uint32) bool {
	rl := len(rangePairs)
	for i := 0; i < rl; i += 2 {
...
func is_ident1(c uint32) bool {
	return in_range(range_ident1, c)
}

var range_ident2 = []uint32{
...
func is_ident2(c uint32) bool {
	return in_range(range_ident1, c) || in_range(range_ident2, c)
}
...
func char_width(c uint32) int {

	if in_range(range_w0, c) {
		return 0
	}

	if in_range(range_w2, c) {
		return 2
	}
...
func display_width(buf []byte, pos int, len int) int {
	start := pos
	w := 0
	var c uint32
