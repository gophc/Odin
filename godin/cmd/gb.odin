...
memcopy :: proc(dest, source: rawptr, n: isize) -> rawptr {
    if dest == nil || source == nil || n <= 0 {
        return nil
    }

    dest_addr := uintptr(dest)
    source_addr := uintptr(source)
    len_u := uintptr(n)
    has_overlap := false

    if source_addr <= dest_addr && dest_addr < source_addr + len_u {
        has_overlap = true
    } else if dest_addr <= source_addr && source_addr < dest_addr + len_u {
        has_overlap = true
    }
    ...
    if has_overlap {
        ...
        mem.copy_non_overlapping(dest, source, int(n))
    }
    return dest
}
...
fprintf :: proc(f: io.Writer, format: string, args: ..any) {
    if f == nil do return
    s := fmt.tprintf(format, ..args)
    io.write(f, transmute([]u8)s)
}
...
time_now :: proc() -> f64 {
    t := time.now()
    unix_nanos := time.time_to_unix_nano(t)
    // Convert to seconds as a double
    return f64(unix_nanos) * 1e-9 // approximate
}
gb_utc_time_now_odin :: proc() -> i64 {
    t := time.now()
    // 转换为 Unix 时间戳（纳秒）
    unix_nanos := time.time_to_unix_nano(t)
    // 转换为微秒 (1 微秒 = 1000 纳秒)
    return unix_nanos / 1_000
}
