...
// ===== Part 1: Core types, char utils, memchr, sort, endian =====
...
gb_printf_err :: proc(fmt: cstring, args: ..any) -> isize // defined in part5

// --- Assert handler ---
@(private)
gb_assert_handler :: proc(prefix, condition, file: cstring, line: i32, msg: cstring, args: ..any) {
	gb_printf_err("%s(%d): %s: ", file, line, prefix)
	if condition != nil && condition[0] != 0 {
		gb_printf_err("`%s` ", condition)
	}
	if msg != nil && msg[0] != 0 {
		gb_printf_err(msg, ..args)
	}
	gb_printf_err("\n")
...
// --- gb_is_power_of_two ---
gb_is_power_of_two :: #force_inline proc(x: isize) -> b32 {
	if x <= 0 do return false
	return (x & (x - 1)) == 0
}

// --- gb_align_forward ---
gb_align_forward :: #force_inline proc(ptr: rawptr, alignment: isize) -> rawptr {
	p := transmute(uintptr)ptr
	return rawptr((p + uintptr(alignment - 1)) &~ uintptr(alignment - 1))
}

// --- gb_memchr (word-accelerated) ---
gb_memchr :: proc(data: rawptr, byte_value: u8, size: isize) -> rawptr {
	s := mem.ptr_offset((^u8)(data), 0)
	n := size
	c := byte_value
	// align to word boundary
	for (transmute(uintptr)s & (size_of(uintptr) - 1)) != 0 && n > 0 && s^ != c {
		s = mem.ptr_offset(s, 1)
		n -= 1
	}
	if n > 0 && s^ != c {
		k := transmute(uintptr)(0x0101010101010101) * uintptr(c)
		w := (^uintptr)(s)
		for n >= isize(size_of(uintptr)) {
			x := w^ ~ k
			t := (x - 0x0101010101010101) & ~x & 0x8080808080808080
			if t != 0 do break
			w = mem.ptr_offset(w, 1)
			n -= isize(size_of(uintptr))
		}
		s = (^u8)(w)
		for n > 0 && s^ != c {
			s = mem.ptr_offset(s, 1)
	...
	if n > 0 do return rawptr(s)
	return nil
}

// --- gb_memrchr ---
gb_memrchr :: proc(data: rawptr, byte_value: u8, size: isize) -> rawptr {
	s := (^u8)(data)
...
gb_strlen :: proc(str: cstring) -> isize {
	...
	s := str
	// align to word boundary
	for uintptr(uintptr(s)) % size_of(uintptr) != 0 {
		if s[0] == 0 do return isize(uintptr(s) - uintptr(begin))
		s = cstring(&s[1])
	}
	w := (^uintptr)(s)
	for {
	...
	return isize(uintptr(s) - uintptr(begin))
}

// gb_strnlen: compute length of string up to max_len
gb_strnlen :: proc(str: cstring, max_len: isize) -> isize {
	end_ptr := gb_memchr(rawptr(str), 0, max_len)
	if end_ptr != nil {
		return isize(uintptr(end_ptr) - uintptr(str))
	}
	return max_len
}
...
gb_strlcpy :: proc(dest: cstring, source: cstring, len: isize) -> isize {
	...
	return isize(uintptr(s) - uintptr(source_start))
}
...
gb_str_has_suffix :: proc(str, suffix: cstring) -> b32 {
	...
	j := gb_strlen(suffix)
	if j <= i {
		return gb_strcmp(cstring(&str[i - j]), suffix) == 0
	}
	return false
}
...
gb_str_concat :: proc(dest: cstring, dest_len: isize, src_a: cstring, src_a_len: isize, src_b: cstring, src_b_len: isize) {
	...
	mem.copy(rawptr(&dest[src_a_len]), rawptr(src_b), int(src_b_len))
...
// ===== Part 3: String conversion + UTF functions =====
...
@(private)
_gb_scan_u64 :: proc(text: cstring, base: i32, value: ^u64) -> isize {
	text_begin := text
	result: u64 = 0
	s := text
	if base == 16 && len(s) >= 2 && s[0] == '0' && s[1] == 'x' {
		s = cstring(&s[2])
	}
	for {
		v: u64
	...
	if value != nil do value^ = result
	return isize(uintptr(s) - uintptr(text_begin))
}

@(private)
_gb_scan_i64 :: proc(text: cstring, base: i32, value: ^i64) -> isize {
	text_begin := text
	result: i64 = 0
	negative := false
	s := text
...
gb_str_to_u64 :: proc(str: cstring, end_ptr: ^cstring, base: i32) -> u64 {
	b := base
	if b == 0 {
	...
	value: u64 = 0
	n := _gb_scan_u64(str, b, &value)
	if end_ptr != nil do end_ptr^ = cstring(&str[n])
	return value
}

gb_str_to_i64 :: proc(str: cstring, end_ptr: ^cstring, base: i32) -> i64 {
	b := base
	if b == 0 {
		if len(str) > 2 && str[0] == '0' && str[1] == 'x' {
			b = 16
	...
	value: i64 = 0
	n := _gb_scan_i64(str, b, &value)
	if end_ptr != nil do end_ptr^ = cstring(&str[n])
	return value
}

gb_str_to_f32 :: proc(str: cstring, end_ptr: ^cstring) -> f32 {
	f := gb_str_to_f64(str, end_ptr)
	return f32(f)
}

gb_str_to_f64 :: proc(str: cstring, end_ptr: ^cstring) -> f64 {
	s := str
	// skip spaces
	...
	if end_ptr != nil do end_ptr^ = s
	return result
}

// --- Number to string ---

gb_i64_to_str :: proc(value: i64, string: cstring, base: i32) {
...
gb_u64_to_str :: proc(value: u64, string: cstring, base: i32) {
	buf := string
	v := value
	if v != 0 {
		for v > 0 {
...
gb_utf8_strlen :: proc(str: ^u8) -> isize {
	count: isize = 0
	s := str
	for s^ != 0 {
		c := s^
...
gb_utf8_strnlen :: proc(str: ^u8, max_len: isize) -> isize {
	count: isize = 0
	s := str
	remaining := max_len
	for s^ != 0 && remaining > 0 {
...
gb_utf8_to_ucs2 :: proc(buffer: ^u16, len: isize, str: ^u8) -> ^u16 {
	c: Rune
	i: isize = 0
	rem := len - 1
	s := str
	for s^ != 0 {
		if i >= rem do return nil
		if (s^ & 0x80) == 0 {
			buffer[i] = u16(s^)
			i += 1
			s = mem.ptr_offset(s, 1)
		} else if (s^ & 0xe0) == 0xc0 {
			if s^ < 0xc2 do return nil
			c = Rune(s^ & 0x1f) << 6
			s = mem.ptr_offset(s, 1)
			if (s^ & 0xc0) != 0x80 do return nil
			buffer[i] = u16(c + Rune(s^ & 0x3f))
			i += 1
			s = mem.ptr_offset(s, 1)
		} else if (s^ & 0xf0) == 0xe0 {
			s1 := mem.ptr_offset(s, 1)
			if s^ == 0xe0 && (s1^ < 0xa0 || s1^ > 0xbf) do return nil
			if s^ == 0xed && s1^ > 0x9f do return nil
			c = Rune(s^ & 0x0f) << 12
			s = mem.ptr_offset(s, 1)
			if (s^ & 0xc0) != 0x80 do return nil
			c += Rune(s^ & 0x3f) << 6
			...
			if s^ > 0xf4 do return nil
			s1 := mem.ptr_offset(s, 1)
			if s^ == 0xf0 && (s1^ < 0x90 || s1^ > 0xbf) do return nil
			if s^ == 0xf4 && s1^ > 0x8f do return nil
			c = Rune(s^ & 0x07) << 18
			s = mem.ptr_offset(s, 1)
			if (s^ & 0xc0) != 0x80 do return nil
			c += Rune(s^ & 0x3f) << 12
			...
			c += Rune(s^ & 0x3f)
			s = mem.ptr_offset(s, 1)
			if (c & 0xfffff800) == 0xd800 do return nil
			if c >= 0x10000 {
				c -= 0x10000
				if i + 2 > rem do return nil
				buffer[i] = u16(0xd800 | (0x3ff & (c >> 10)))
				i += 1
				buffer[i] = u16(0xdc00 | (0x3ff & c))
				i += 1
			}
		} else {
			return nil
...
gb_ucs2_to_utf8 :: proc(buffer: ^u8, len: isize, str: ^u16) -> ^u8 {
	i: isize = 0
	rem := len - 1
	s := str
	for s^ != 0 {
		if s^ < 0x80 {
			if i + 1 > rem do return nil
			buffer[i] = u8(s^)
			i += 1
			s = mem.ptr_offset(s, 1)
		} else if s^ < 0x800 {
			if i + 2 > rem do return nil
			buffer[i] = u8(0xc0 + (s^ >> 6))
			i += 1
			buffer[i] = u8(0x80 + (s^ & 0x3f))
			i += 1
			s = mem.ptr_offset(s, 1)
		} else if s^ >= 0xd800 && s^ < 0xdc00 {
			if i + 4 > rem do return nil
			s1 := mem.ptr_offset(s, 1)
			c := Rune(s^ - 0xd800) << 10 + Rune(s1^ - 0xdc00) + 0x10000
			buffer[i] = u8(0xf0 + (c >> 18))
			i += 1
			buffer[i] = u8(0x80 + ((c >> 12) & 0x3f))
			i += 1
			buffer[i] = u8(0x80 + ((c >> 6) & 0x3f))
			i += 1
			buffer[i] = u8(0x80 + (c & 0x3f))
			i += 1
			s = mem.ptr_offset(s, 2)
		} else if s^ >= 0xdc00 && s^ < 0xe000 {
			return nil
		} else {
			if i + 3 > rem do return nil
			buffer[i] = u8(0xe0 + (s^ >> 12))
			i += 1
			buffer[i] = u8(0x80 + ((s^ >> 6) & 0x3f))
			i += 1
			buffer[i] = u8(0x80 + (s^ & 0x3f))
			i += 1
			s = mem.ptr_offset(s, 1)
...
gb_utf8_to_ucs2_buf :: proc(str: ^u8) -> ^u16 {
	return gb_utf8_to_ucs2(&UTF16_BUF[0], 4096, str)
}

@(private)
UTF8_BUF: [4096]u8

gb_ucs2_to_utf8_buf :: proc(str: ^u16) -> ^u8 {
	return gb_ucs2_to_utf8(&UTF8_BUF[0], 4096, str)
}
...
// --- gb_utf8_decode ---
gb_utf8_decode :: proc(str: ^u8, str_len: isize, codepoint_out: ^Rune) -> isize {
	width: isize = 0
	...
	if codepoint_out != nil do codepoint_out^ = codepoint
	return width
}

// --- gb_utf8_codepoint_size ---
gb_utf8_codepoint_size :: proc(str: ^u8, str_len: isize) -> isize {
...
gb_adler32 :: proc(data: rawptr, len: isize) -> u32 {
	...
	for remaining > 0 {
		...
		for i = 0; i + 7 < block_len; i += 8 {
			...
			a += u32(bytes[6]); b += a
			a += u32(bytes[7]); b += a
			bytes = mem.ptr_offset(bytes, 8)
		}
		for ; i < block_len; i += 1 {
			a += u32(bytes^)
			b += a
			bytes = mem.ptr_offset(bytes, 1)
		}
		a %= MOD_ADLER
		b %= MOD_ADLER
		remaining -= block_len
...
gb_murmur32_seed :: proc(data: rawptr, len: isize, seed: u32) -> u32 {
	...
	hash := seed
	blocks := (^u32)(data)
	tail := mem.ptr_offset((^u8)(data), nblocks * 4)

	for i: isize = 0; i < nblocks; i += 1 {
		k := blocks^
		blocks = mem.ptr_offset(blocks, 1)
		k *= C1
		k = (k << R1) | (k >> (32 - R1))
		k *= C2
		hash ~= k
...
gb_murmur64_seed :: proc(data: rawptr, len: isize, seed: u64) -> u64 {
	...
	h := seed ~ (u64(len) * M)
	blocks := (^u64)(data)
	end_ptr := mem.ptr_offset(blocks, len / 8)
	tail := (^u8)(end_ptr)
...
// ===== Part 5: Path, printf, system/time =====
// --- Path functions (Windows-style) ---

gb_path_is_absolute :: proc(path: cstring) -> b32 {
	if path == nil do return false
	ln := gb_strlen(path)
	return ln > 2 && gb_char_is_alpha(path[0]) && path[1] == ':' && path[2] == '\\'
}

gb_path_is_relative :: proc(path: cstring) -> b32 {
	return !gb_path_is_absolute(path)
}

gb_path_is_root :: proc(path: cstring) -> b32 {
	if path == nil do return false
	return gb_path_is_absolute(path) && gb_strlen(path) == 3
}

gb_path_base_name :: proc(path: cstring) -> cstring {
	if path == nil do return nil
	ls := gb_char_last_occurence(path, '/')
	if ls == nil do return path
	return cstring(&ls[1])
}

gb_path_extension :: proc(path: cstring) -> cstring {
	if path == nil do return nil
	ld := gb_char_last_occurence(path, '.')
	if ld == nil do return nil
	return cstring(&ld[1])
}

// gb_path_get_full_name: uses Windows GetFullPathNameW via Odin's sys/windows
gb_path_get_full_name :: proc(path: cstring, loc := #caller_location) -> cstring {
	// Convert to wide string
	w_path := windows.utf8_to_wstring(string(path))
	defer delete(w_path)

	w_len := windows.GetFullPathNameW(w_path, 0, nil, nil)
	if w_len == 0 do return nil

	w_fullpath := make([]u16, w_len + 1)
	defer delete(w_fullpath)

	windows.GetFullPathNameW(w_path, w_len, &w_fullpath[0], nil)
	w_fullpath[w_len] = 0

	// Convert back to utf8
	new_len := windows.WideCharToMultiByte(windows.CP_UTF8, 0x00000080, &w_fullpath[0], i32(w_len), nil, 0, nil, nil)
	if new_len == 0 do return nil

	new_path := make([]byte, new_len + 1)
	new_len1 := windows.WideCharToMultiByte(windows.CP_UTF8, 0x00000080, &w_fullpath[0], i32(w_len), &new_path[0], new_len, nil, nil)
	if new_len1 == 0 {
		delete(new_path)
		return nil
	}
	new_path[new_len] = 0
	return cstring(&new_path[0])
}

// --- Printf functions ---
// These wrap Odin's fmt package to provide C-like printf behavior.

gb_printf :: proc(fmt_str: cstring, args: ..any) -> isize {
	return gb_fprintf(nil, fmt_str, ..args)
}

gb_printf_err :: proc(fmt_str: cstring, args: ..any) -> isize {
	res := fmt.eprintf(string(fmt_str), ..args)
	return isize(res)
}

gb_fprintf :: proc(f: rawptr, fmt_str: cstring, args: ..any) -> isize {
	res := fmt.printf(string(fmt_str), ..args)
	return isize(res)
}

gb_bprintf :: proc(fmt_str: cstring, args: ..any) -> cstring {
	// Use a thread-local static buffer (simplified)
	// The C version uses a thread-local static buffer of 4096 bytes
	str := fmt.aprintf(string(fmt_str), ..args)
	return cstring(&str[0])  // Note: caller should NOT free this
	// In practice, the C version returns a static buffer, but Odin convention
	// returns allocated strings. We keep the same behavior for compatibility.
}

gb_snprintf :: proc(str: cstring, n: isize, fmt_str: cstring, args: ..any) -> isize {
	s := fmt.bprintf([]byte(str[0:n]), string(fmt_str), ..args)
	if len(s) < int(n) {
		str[len(s)] = 0
	}
	return isize(len(s))
...
// --- va_list variants ---
// Odin doesn't have va_list; these are thin wrappers around the varargs versions.

gb_printf_va :: proc(fmt_str: cstring, va: rawptr) -> isize {
	// va_list is not supported in Odin; stub that delegates to gb_printf
	return gb_printf(fmt_str)
}

gb_printf_err_va :: proc(fmt_str: cstring, va: rawptr) -> isize {
	return gb_printf_err(fmt_str)
}

gb_fprintf_va :: proc(f: rawptr, fmt_str: cstring, va: rawptr) -> isize {
	return gb_fprintf(f, fmt_str)
}

gb_bprintf_va :: proc(fmt_str: cstring, va: rawptr) -> cstring {
	return gb_bprintf(fmt_str)
}

gb_snprintf_va :: proc(str: cstring, n: isize, fmt_str: cstring, va: rawptr) -> isize {
	return gb_snprintf(str, n, fmt_str)
}

// --- Time functions ---

gb_rdtsc :: proc() -> u64 {
	// RDTSC is x86-specific; on other platforms, return 0 with comment
	// Not implemented: requires x86 intrinsics
	return 0
}
...
gb_utc_time_now :: proc() -> u64 {
	// Microseconds since Windows epoch (Jan 1, 1601)
	// Use Odin time, converted
	t := time.now()
	// This is a simplified version; the C code uses GetSystemTimeAsFileTime
	return u64(time.to_unix_nanoseconds(t)) / 100 + 11644473600000000
}
...
gb_exit :: proc(code: u32) {
	os.exit(int(code))
}

gb_get_env :: proc(name: cstring) -> cstring {
	if name == nil || name[0] == 0 do return nil
	val := os.get_env(string(name))
	if val == "" do return nil
	// Return newly allocated string (caller owns it)
...
gb_set_env :: proc(name, value: cstring) {
	os.set_env(string(name), string(value))
}

gb_unset_env :: proc(name: cstring) {
	os.unset_env(string(name))
}
