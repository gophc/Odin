...
array_end_ptr :: proc(arr: ^[dynamic]$T) -> ^T {
	if len(arr) > 0 do return &arr[len(arr) - 1]
	return nil
}
...
recursive_mutex_lock :: proc(m: ^RecursiveMutex) {
	sync.lock(m)
}
recursive_mutex_unlock :: proc(m: ^RecursiveMutex) {
	sync.unlock(m)
}
...
range_cache_add_index :: proc(c: ^RangeCache, idx: i64) -> bool {
	...
	append(in &c.ranges, RangeValue{ lo = idx, hi = idx })
	return false
}
range_cache_add_range :: proc(c: ^RangeCache, lo, hi: i64) -> bool {
	...
	append(in &c.ranges, RangeValue{ lo = lo, hi = hi })
	return false
}
