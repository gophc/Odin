...
@(test)
test_gbstring :: proc(t: ^testing.T) {
    s := make_string("hello")
    defer string_free(s)
    testing.expect(t, string_length(s) == 5)
    string_appendc(&s, " world")
    testing.expect(t, string(s) == "hello world")
    string_trimspace := string_trim_space(s)
    defer string_free(string_trimspace)
    testing.expect(t, string(string_trimspace.data) == "hello world") // unchanged
    testing.expect(t, string_are_equal(s, string_trimspace))
}
...
@(test)
test_file_operations :: proc(t: ^testing.T) {
    filename := "test_gb_iodin.txt"
    defer os.remove(filename)

    // Create and write
    f := file_create(filename)
    if f == nil {
        testing.expect(t, false, "failed to create file")
        return
    }
    written := file_write(f, transmute([]u8)("Hello, Odin!"))
    testing.expect(t, written)
    file_close(f)

    // Open read
    f2 := file_open(filename)
    if f2 == nil {
        testing.expect(t, false, "failed to open file")
        return
    }
    data := make([]u8, 20, context.temp_allocator)
    read_ok := file_read(f2, data[:12])
    testing.expect(t, read_ok)
    testing.expect(t, string(data[:12]) == "Hello, Odin!")
    file_close(f2)

    testing.expect(t, file_exists(filename))
    testing.expect(t, file_size(f2) == 12)
}
